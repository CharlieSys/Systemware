...
["/auth.json","/system.json","/servercfg.json","/webhooks.json"].forEach(file => {
...
bot.on("guildMemberAdd", (guild, member) => {
	if(system[member.id] && config[guild.id].rolesEnabled) {
		system[member.id].forEach(tul => {
...
bot.on("guildMemberRemove", (guild, member) => {
	if(system[member.id] && config[guild.id].rolesEnabled) {
		system[member.id].filter(t => t.roles && t.roles[guild.id]).forEach(tul => {
...
bot.on("messageCreate", async function (msg) {
	...
	let cfg = msg.channel.guild && config[msg.channel.guild.id] || { prefix: "s!", rolesEnabled: false, lang: "system"};
	if (msg.content.startsWith(cfg.prefix) && (!cfg.cmdblacklist || !cfg.cmdblacklist.includes(msg.channel.id))) {
	...
	} else if(system[msg.author.id] && !(msg.channel instanceof Eris.PrivateChannel) && (!cfg.blacklist || !cfg.blacklist.includes(msg.channel.id))) {
		...
		for(let i = 0; i < lines.length; i++) {
			let found = false;
			system[msg.author.id].forEach(t => {
				if(checkAlter(msg, t, cleanarr[i])) {
		...
		if(!replace[0]) {
			for(let s of system[msg.author.id]) {
				if(checkAlter(msg, t, clean)) {
...
async function replaceMessage(msg, cfg, system, content) {
	const hook = await fetchWebhook(msg.channel);
	const data = {
		wait: true,
		content: content,
		username: `${alter.name} ${tulpa.tag ? tulpa.tag : ""} ${checkTulpaBirthday(tulpa) ? "\uD83C\uDF70" : ""}`,
		avatarURL: alter.url,
	};
	...
	if(msg.attachments[0]) {
		return sendAttachmentsWebhook(msg, cfg, data, content, hook, system);
	}
